...
- **Bilibili** — WBI-signed API; default `--quality 360p` for no-login reliability, higher qualities may require login or may fail; official Bilibili AI video summaries are fetched from `/x/web-interface/view/conclusion/get` when a logged-in cookie is available (`OMNIFETCH_BILIBILI_COOKIE`, `BILIBILI_COOKIE`, `BILI_COOKIE`, or `SESSDATA`); subtitles via yt-dlp.
